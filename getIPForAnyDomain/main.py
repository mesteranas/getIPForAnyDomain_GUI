...
class main (qt.QMainWindow):
    def __init__(self):
        ...
        layout.addWidget(qt.QLabel(_("inter domain")))
        self.domain=qt.QLineEdit()
        layout.addWidget(self.domain)
        self.chec=qt.QPushButton(_("get"))
        self.chec.setDefault(True)
        self.chec.clicked.connect(self.on_chec)
        layout.addWidget(self.chec)
    ...
    def on_chec(self):
        try:
            IP=socket.gethostbyname(self.domain.text())
        except:
            IP="error"
        self.result.setText(IP)
